...
function Dung(_game) {
	...
	this.initialize = function(_game) {
		...
		game = _game;
		img = new ImageList("images/billun ("+randomBH(1,4)+").gif", 150, 150);
...
function Man(_game) {
	...
	this.initialize = function(_game) {
		...
		img = new ImageList("images/man.gif", 150, 150);
	...
	this.collision = function(dung_x) {
		return (Math.abs(dung_x - (posx + 150 / 2)) < 56);//끝의 숫자값의 따라 충돌정도가 달라짐
	};
	...
	this.spawn = function() {
		...
		this.setLeft(parseInt((game.size[0] - 90) / 2));
		this.run(null);
	};
